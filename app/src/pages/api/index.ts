...
import { encryptData, getAuthPersonInfo, prepareAuthParams } from '../../utils/oAuthHandler'
...
/**
 * Get OneDrive API access token.
 * If a valid token exists in storage, return it.
 * Otherwise, acquire a new one based on the configured authentication type.
 *
 * @returns {Promise<string>} Access token for OneDrive API
 */
export async function getAccessToken(): Promise<string> {
  const { accessToken, refreshToken } = await getOdAuthTokens()

  // If a valid access token exists in storage, return it
  if (typeof accessToken === 'string') {
    return accessToken
  }

  if (typeof refreshToken !== 'string') {
    console.log('No refresh token found, re-authentication required.')
    return ''
  }

  // Creat generic request body
  const body = await prepareAuthParams()
  ...
  try {
    // Send request to OneDrive API
    const resp = await axios.post(apiConfig.authApi, body, {
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
    })

    // Process response
    if ('access_token' in resp.data && 'expires_in' in resp.data) {
      const { expires_in, refresh_token, access_token } = resp.data
      await storeOdAuthTokens({
        accessToken: access_token,
        accessTokenExpiry: parseInt(expires_in),
        refreshToken: refresh_token
      })
      console.log(`Obtained access token using ${apiConfig.authType}.`)
      return access_token
    }
  } catch (error: any) {
